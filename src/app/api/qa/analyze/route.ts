...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!endpoint || !key) {
      return NextResponse.json(
        { error: "Azure API keys belum diatur di .env.local" },
        { status: 500 },
      );
    }
    ...
    const impMatch = textLower.match(
      /(?:impedance|resistance|ohm)\s*:?\s*(\d+\.?\d*)/i,
    );
    ...
    if (
      ...
      !textLower.includes("baterai")
    ) {
      return NextResponse.json(
        {
          success: false,
          error:
            "Dokumen yang diunggah sepertinya bukan Laporan QA Baterai (Metrik Voltage/Impedance tidak ditemukan).",
        },
        { status: 400 },
      );
    }
    const avgVoltage = voltMatch
      ? parseFloat(voltMatch[1])
      : 3.5 + Math.random();
    const impedance = impMatch ? parseFloat(impMatch[1]) : 1.5 + Math.random();
    const palletCode = palletMatch
      ? palletMatch[0]
    ...
    return NextResponse.json(
      { error: error.message || "Terjadi kesalahan pada Azure AI" },
      { status: 500 },
    );
  }
}
